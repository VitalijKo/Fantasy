...
from fantasy.face_cache import get_faces_cache, set_faces_cache
...
def get_face_analyser():
	...
	with THREAD_LOCK:
		if FACE_ANALYSER is None:
			FACE_ANALYSER = insightface.app.FaceAnalysis(name='buffalo_l', providers=fantasy.globals.execution_providers)
...
def get_many_faces(frame):
	try:
		...
		if fantasy.globals.face_analyser_direction:
			faces = sort_by_direction(faces, fantasy.globals.face_analyser_direction)

		if fantasy.globals.face_analyser_age:
			faces = filter_by_age(faces, fantasy.globals.face_analyser_age)

		if fantasy.globals.face_analyser_gender:
			faces = filter_by_gender(faces, fantasy.globals.face_analyser_gender)
