...
from fantasy.vision import get_video_frame, normalize_frame_color, read_static_image
from fantasy.face_analyser import get_many_faces
from fantasy.face_reference import clear_face_reference
from fantasy.ui import core as ui
from fantasy.utilities import is_image, is_video
...
def render():
	...
	reference_face_gallery_args = {
		...
		'columns': 10,
		'allow_preview': False,
		'visible': 'reference' in fantasy.globals.face_recognition
	}

	if is_image(fantasy.globals.target_path):
		reference_frame = read_static_image(fantasy.globals.target_path)
	...
	FACE_RECOGNITION_DROPDOWN = gradio.Dropdown(
		...
		choices=fantasy.choices.face_recognition,
		value=fantasy.globals.face_recognition
	)
	REFERENCE_FACE_POSITION_GALLERY = gradio.Gallery(**reference_face_gallery_args)
	REFERENCE_FACE_DISTANCE_SLIDER = gradio.Slider(
		label=wording.get('reference_face_distance_slider_label'),
		value=fantasy.globals.reference_face_distance,
		maximum=3,
		step=0.05,
		visible = 'reference' in fantasy.globals.face_recognition
	)
...
def update_face_recognition(face_recognition):
	if face_recognition == 'reference':
		fantasy.globals.face_recognition = face_recognition
...
def update_face_reference_position(reference_face_position=0):
	...
	fantasy.globals.reference_face_position = reference_face_position
...
def update_reference_face_distance(reference_face_distance):
	fantasy.globals.reference_face_distance = reference_face_distance
