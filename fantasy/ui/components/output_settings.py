...
def render():
	...
	OUTPUT_IMAGE_QUALITY_SLIDER = gradio.Slider(
		...
		value=fantasy.globals.output_image_quality,
		step=1,
		visible=is_image(fantasy.globals.target_path)
	)
	OUTPUT_VIDEO_ENCODER_DROPDOWN = gradio.Dropdown(
		label=wording.get('output_video_encoder_dropdown_label'),
		choices=fantasy.choices.output_video_encoder,
		value=fantasy.globals.output_video_encoder,
		visible=is_video(fantasy.globals.target_path)
	)
	OUTPUT_VIDEO_QUALITY_SLIDER = gradio.Slider(
		label=wording.get('output_video_quality_slider_label'),
		value=fantasy.globals.output_video_quality,
		step=1,
		visible=is_video(fantasy.globals.target_path)
	)
