...
from fantasy.vision import detect_fps

TEMP_DIRECTORY_PATH = os.path.join(tempfile.gettempdir(), 'fantasy')
...
def open_ffmpeg(args):
	commands = ['ffmpeg', '-hide_banner', '-loglevel', 'error'] + args

	return subprocess.Popen(commands, stdin=subprocess.PIPE)


def extract_frames(target_path, fps):
	temp_frame_compression = round(31 - fantasy.globals.temp_frame_quality * 0.31)
	trim_frame_start = fantasy.globals.trim_frame_start
	trim_frame_end = fantasy.globals.trim_frame_end
...
def compress_image(output_path):
	output_image_compression = round(31 - fantasy.globals.output_image_quality * 0.31)
...
def merge_video(target_path, fps):
	temp_output_video_path = get_temp_output_video_path(target_path)

	temp_frames_pattern = get_temp_frames_pattern(target_path, '%04d')

	commands = ['-hwaccel', 'auto', '-r', str(fps), '-i', temp_frames_pattern, '-c:v', fantasy.globals.output_video_encoder]

	if fantasy.globals.output_video_encoder in ['libx264', 'libx265']:
		output_video_compression = round(51 - fantasy.globals.output_video_quality * 0.5)

		commands.extend(['-crf', str(output_video_compression)])

	if fantasy.globals.output_video_encoder in [ 'libvpx-vp9' ]:
		output_video_compression = round(63 - fantasy.globals.output_video_quality * 0.5)

		commands.extend(['-crf', str(output_video_compression)])

	if fantasy.globals.output_video_encoder in [ 'h264_nvenc', 'hevc_nvenc' ]:
		output_video_compression = round(51 - fantasy.globals.output_video_quality * 0.5)
...
def get_temp_frames_pattern(target_path, temp_frame_prefix):
	...
	return os.path.join(temp_directory_path, temp_frame_prefix + '.' + fantasy.globals.temp_frame_format)
...
def get_temp_output_video_path(target_path):
	temp_directory_path = get_temp_directory_path(target_path)
...
def clear_temp(target_path):
	...
	if not fantasy.globals.keep_temp and is_directory(temp_directory_path):
...
def is_download_done(url, file_path):
...
def get_device(execution_providers):
