...
def load_frame_processor_module(frame_processor):
	try:
		frame_processor_module = importlib.import_module(f'vitswap.processors.frame.modules.{frame_processor}')
...
def multi_process_frames(source_path, temp_frame_paths, process_frames):
	...
	with tqdm(total=len(temp_frame_paths), desc=wording.get('processing'), unit='frame', dynamic_ncols=True, bar_format=progress_bar_format) as progress:
		with ThreadPoolExecutor(max_workers=vitswap.globals.execution_thread_count) as executor:
			futures = []

			queue_temp_frame_paths = create_queue(temp_frame_paths)

			queue_per_future = max(len(temp_frame_paths) // vitswap.globals.execution_thread_count * vitswap.globals.execution_queue_count, 1)
...
def update_progress(progress=None):
	...
	progress.set_postfix({
		...
		'execution_providers': vitswap.globals.execution_providers,
		'execution_thread_count': vitswap.globals.execution_thread_count,
		'execution_queue_count': vitswap.globals.execution_queue_count
	})
