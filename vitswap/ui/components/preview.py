...
from vitswap.vision import get_video_frame, count_video_frame_total, normalize_frame_color, resize_frame_dimension, read_static_image
from vitswap.face_analyser import get_one_face
from vitswap.face_reference import get_face_reference, set_face_reference
from vitswap.processors.frame.core import load_frame_processor_module
from vitswap.ui import core as ui
from vitswap.utilities import is_video, is_image
...
def render():
	...
	source_face = get_one_face(read_static_image(vitswap.globals.source_path))
	...
	if is_image(vitswap.globals.target_path):
		target_frame = read_static_image(vitswap.globals.target_path)
	...
	if is_video(vitswap.globals.target_path):
		temp_frame = get_video_frame(vitswap.globals.target_path, vitswap.globals.reference_frame_number)
		...
		preview_frame_slider_args['value'] = vitswap.globals.reference_frame_number
		preview_frame_slider_args['maximum'] = count_video_frame_total(vitswap.globals.target_path)
...
def update_preview_image(frame_number=0):
	...
	if is_video(vitswap.globals.target_path):
		vitswap.globals.reference_frame_number = frame_number
...
def update_preview_frame_slider(frame_number=0):
	...
	if is_video(vitswap.globals.target_path):
		...
		video_frame_total = count_video_frame_total(vitswap.globals.target_path)
...
def process_preview_frame(source_face, reference_face, temp_frame):
	...
	for frame_processor in vitswap.globals.frame_processors:
