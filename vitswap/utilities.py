...
from vitswap.vision import detect_fps

TEMP_DIRECTORY_PATH = os.path.join(tempfile.gettempdir(), 'vitswap')
...
def extract_frames(target_path, fps):
	temp_frame_compression = round(31 - vitswap.globals.temp_frame_quality * 0.31)
	trim_frame_start = vitswap.globals.trim_frame_start
	trim_frame_end = vitswap.globals.trim_frame_end
...
def compress_image(output_path):
	output_image_compression = round(31 - vitswap.globals.output_image_quality * 0.31)
...
def merge_video(target_path, fps):
	...
	commands = ['-hwaccel', 'auto', '-r', str(fps), '-i', temp_frames_pattern, '-c:v', vitswap.globals.output_video_encoder]

	if vitswap.globals.output_video_encoder in ['libx264', 'libx265']:
		output_video_compression = round(51 - vitswap.globals.output_video_quality * 0.5)

		commands.extend(['-crf', str(output_video_compression)])

	if vitswap.globals.output_video_encoder in [ 'libvpx-vp9' ]:
		output_video_compression = round(63 - vitswap.globals.output_video_quality * 0.5)

		commands.extend(['-crf', str(output_video_compression)])

	if vitswap.globals.output_video_encoder in [ 'h264_nvenc', 'hevc_nvenc' ]:
		output_video_compression = round(51 - vitswap.globals.output_video_quality * 0.5)
...
def get_temp_frames_pattern(target_path, temp_frame_prefix):
	...
	return os.path.join(temp_directory_path, temp_frame_prefix + '.' + vitswap.globals.temp_frame_format)
...
def clear_temp(target_path):
	...
	if not vitswap.globals.keep_temp and is_directory(temp_directory_path):
